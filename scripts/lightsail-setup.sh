...
echo "Next:"
echo "  Staging:"
echo "    1. Clone repo to /opt/swimit"
echo "    2. cp .env.lightsail.example .env && edit DOMAIN + passwords"
echo "    3. Deploy from PC: bash scripts/deploy-staging-from-pc.sh"
echo "  Production (app.swimit.co.in):"
echo "    1. Clone repo to /opt/swimit (or let deploy-prod-from-pc.sh clone it)"
echo "    2. cp .env.lightsail.prod.example .env && edit DOMAIN + passwords"
echo "    3. Deploy from PC: SWIMIT_PROD_HOST=<static-ip> bash scripts/deploy-prod-from-pc.sh"
